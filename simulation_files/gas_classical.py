...
def mom_angmom_rescale(simulation):
    ...
    desiredKE = 3*atom_count*kb*avogadro/1000*temperature/2 #(3/2)*NkT converted to kJ/mol
...
prefactor = np.sqrt(kb*temperature/masses) / 1000 #convert to nm/ps from m/s
...
simulation.context.setVelocities(mom_angmom_rescale(simulation))

simulation.step(2000000) #1 ns initial simulation

simulation.reporters.append(StateDataReporter(False,"gas_thermo.avg", thermo_freq, step=True, time=True, density=True, totalEnergy=True, kineticEnergy=True, volume=True, potentialEnergy=True, temperature=True))
simulation.reporters.append(PDBReporter('gas_trajectory.lammpstrj',print_velocities,coords_freq))
...
for batch in range(batch_count):
    indices = np.where(np.random.uniform(low=0, high=1, size=(atom_count*batch_size,))<collision_probability)
    indices = np.array([(i//atom_count,i%atom_count) for i in indices[0]])
    if len(indices)==0:
        indices = np.array([(100000000,1000000000)])
    for simstep in range(batch_size):
        if simstep in indices[:,0]:
            ...
            velocity = np.array([np.array([x.value_in_unit(nanometer/picosecond) for x in i]) for i in state.getVelocities()]) #simulation.context.getState(getVelocities=True).getVelocities()
            for row in step_array:
                velocity[row[1]] = np.random.normal(0,1,3)*prefactor[row[1]]
            velocity = Quantity(value=velocity, unit=nanometer/picosecond)
            simulation.context.setVelocities(velocity)
        simulation.step(1)
        step_count += 1
        if step_count % 2000 == 0:
            reset_velocity = mom_angmom_rescale(simulation)
            simulation.context.setVelocities(reset_velocity)
print('4 ns Data Collection:', time.time()-st)
Restart.save_simulation('gas_classical.save',simulation,'classical')
