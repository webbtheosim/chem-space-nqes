...
# Initialize dictionary to store thermodynamic data
gas_thermo_dict = {}

# Loop through each molecule
for molecule in molecules[:]:
    # Process Classical simulation data
    ...
    # Calculate average potential energy over 1000-frame blocks
    gas_thermo_dict[molecule,'classical','Potential Energy'] = [np.mean(thermo_array[:,2][i:i+1000]) for i in list(range(0,4000,1000))]
    # Calculate average total energy over 1000-frame blocks
    gas_thermo_dict[molecule,'classical','Total Energy'] = [np.mean(thermo_array[:,4][i:i+1000]) for i in list(range(0,4000,1000))]

    # Process Path-Integral (PI) simulation data for hydrogenated systems
    gas_thermo_file = open('/projects/WEBB/eser/Gas_RPMD_TAFFI_Simulations/{}/1/gas_thermo.avg'.format(molecule),'r')
    thermo_array = np.loadtxt(gas_thermo_file,delimiter=',')
    # Calculate average kinetic energy sum over 1000-frame blocks
    ...
    # Read total energy from TEoutput file
    ...
    gas_thermo_dict[molecule,'PI_H','Total Energy'] = [np.mean(PIMD_Total_Energy[i:i+1000])/32 for i in list(range(0,4000,1000))]
    # Calculate potential energy from total energy and kinetic energy sums
    ...
    # Process Path-Integral (PI) simulation data for deuterated systems
    ...
    PIMD_Total_Energy = np.array(TE)
    # Calculate average total energy over 1000-frame blocks
    gas_thermo_dict[molecule,'PI_D','Total Energy'] = [np.mean(PIMD_Total_Energy[i:i+1000])/32 for i in list(range(0,4000,1000))]
