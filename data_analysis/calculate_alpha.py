...
# Get simulation type from command line argument
simulation_type = str(sys.argv[1])

# Set parameters based on simulation type
if simulation_type in ['PI_H','PI_D']:
    # Path integral simulation for hydrogen or deuterium
    P = 32 # Number of beads in the path integral simulation
...
# Initialize dictionary to store thermal expansion coefficients
alpha_dict = {}

# Loop through each molecule to calculate thermal expansion coefficients
for molecule in molecules:
    ...
    # Calculate mean alpha and standard error for this molecule
    alpha_dict[molecule,'alpha'] = np.mean(alphas)
    alpha_dict[molecule,'alpha_SEM'] = sem(alphas)

# Save the results for given simulation type
