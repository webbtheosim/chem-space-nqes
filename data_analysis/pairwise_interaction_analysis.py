...
def compute_nonbonded_energy(acceptor_member,donor_member,frame_positions,L):
    """Calculate non-bonded energy (Coulomb + Lennard-Jones) between two atoms"""

    # Get atom types and their properties
    interaction_atom_type = {'acceptor':types[acceptor_member],'donor':types[donor_member]}

    interaction_atom_properties = {}

    # Extract properties (charge, sigma, epsilon) for both atoms
    ...
    # Calculate Lennard-Jones parameters using mixing rules
    ...
    *((interaction_atom_properties['acceptor','sigma']**3)*(interaction_atom_properties['donor','sigma']**3))/((interaction_atom_properties['acceptor','sigma']**6)+(interaction_atom_properties['donor','sigma']**6))
    ...
    # Calculate Coulomb energy (converted to kJ/mol)
...
for molecule in [molinput]:
    ...
    # Load hydrogen bond analysis data
    ...
    # Calculate C-O bond vectors for orientation analysis
    ...
    # Calculate pairwise distances between molecules
    ...
    for frame_order,frame in enumerate(analysis_frames): #sample every 1 ns
        ...
        # Process hydrogen bonds for this frame
        ...
        for index1,row in enumerate(AccDon_array):
            for index2,column in enumerate(row):
                residues[index1,index2] = u.atoms[int(AccDon_array[index1,index2])].residue.ix
        ...
        for index,pair in enumerate(residue_pair_list):
            ...
            # Skip pairs that are too far apart (>9 Angstroms)
